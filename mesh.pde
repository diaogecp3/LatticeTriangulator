...
class TriangleMesh {
  ...
  private void setupAdjacentTriangles() {
    if (adjTriangles == null) adjTriangles = new IntList[nv];
    for (int i = 0; i < nv; ++i) {
      adjTriangles[i] = new IntList();
    }
    for (int t = 0; t < nt; ++t) {
      Triangle tri = triangles.get(t);
      adjTriangles[tri.a].append(t);
      adjTriangles[tri.b].append(t);
      adjTriangles[tri.c].append(t);
    }
  }

  void setupVertexNormals() {
    setupAdjacentTriangles();
    assert adjTriangles != null;
    if (vertexNormals == null) vertexNormals = new vec[nv];
    for (int i = 0; i < nv; ++i) {
      IntList adjTris = adjTriangles[i];
      vertexNormals[i] = new vec();  // zero
      for (int t : adjTris) {
        Triangle tri = triangles.get(t);
        pt pa = positions.get(tri.a);
        pt pb = positions.get(tri.b);
        pt pc = positions.get(tri.c);
        vertexNormals[i].add(N(pa, pb, pc));
      }
  ...
  void showVertexNormals(color c) {
    fill(c);
    if (vertexNormals == null) return;
    for (int i = 0; i < nv; ++i) {
      arrow(positions.get(i), V(20, vertexNormals[i]), 2);
    }
  }
