...
class vec {
  ...
  vec rotX(float rad) { float sy=y, c=cos(rad), s=sin(rad); y=y*c-z*s; z=sy*s+z*c; return this; }  // Rotate around axis using right hand rule.
  vec rotY(float rad) { float sz=z, c=cos(rad), s=sin(rad); z=z*c-x*s; x=sz*s+x*c; return this; }
  vec rotZ(float rad) { float sx=x, c=cos(rad), s=sin(rad); x=x*c-y*s; y=sx*s+y*c; return this; }
  vec rot(float rad, vec axisDir) {
    axisDir = axisDir.c().n();
    float c = cos(rad);
    set(c().mul(c).add(cross(axisDir, this).mul(sin(rad))).add(axisDir.mul(dot(axisDir, this) * (1-c))));
    return this;
  }
  vec rot(float rad, vec axisDir, pt pointOnAxis) {
    sub(pointOnAxis);
    rot(rad, axisDir);
    add(pointOnAxis);
    return this;
...
class pt {
  ...
  pt mul(float s, vec v) {sub(v); x*=s; y*=s; z*=s; add(v); return this; }
  pt mul(float s, pt p) {sub(p); x*=s; y*=s; z*=s; add(p); return this; }
  pt snapToSphere(float r) {
    float d = r / sqrt(sq(x) + sq(y) + sq(z));
    x *= d;
    y *= d;
    z *= d;
    return this;
  ...
  pt rotX(float rad) { float sy=y, c=cos(rad), s=sin(rad); y=y*c-z*s; z=sy*s+z*c; return this; }  // Rotate around axis using right hand rule.
  pt rotY(float rad) { float sz=z, c=cos(rad), s=sin(rad); z=z*c-x*s; x=sz*s+x*c; return this; }
  pt rotZ(float rad) { float sx=x, c=cos(rad), s=sin(rad); x=x*c-y*s; y=sx*s+y*c; return this; }
  pt rot(float rad, vec axisDir) {
    axisDir = axisDir.c().n();
    float c = cos(rad);
    set(c().mul(c).add(cross(axisDir, this.toVec()).mul(sin(rad))).add(axisDir.mul(dot(axisDir, this.toVec()) * (1-c))));
    return this;
  }
  pt rot(float rad, vec axisDir, pt pointOnAxis) {
    sub(pointOnAxis);
    rot(rad, axisDir);
    add(pointOnAxis);
    return this;
...
float dist(pt p, pt q) {return d(p, q); }
...
vec cross(vec a, vec b) { return V( a.y*b.z-a.z*b.y, a.z*b.x-a.x*b.z, a.x*b.y-a.y*b.x ); }
float trip(vec a, vec b, vec c) { return dot(a, cross(b, c)); }  // scalar triple product
float signedTetVolume(vec a, vec b, vec c) { return trip(a, b, c) / 6.0; }
vec disp(pt a, pt b) { return V(a, b); }

// Returns angle from A to B around direction D, after projecting A and B onto a plane with normal D.
float angleAroundDirection(vec A, vec B, vec D) {
  vec pA = A.c().sub(D.c().mul(dot(A,D)));
  vec pB = B.c().sub(D.c().mul(dot(B,D)));
  return atan2(dot(cross(D,pA),pB), dot(pA,pB));
}
pt projectOnAxis(pt p, pt axisStart, vec axisDir) { return axisStart.c().add(axisDir.c().mul(dot(disp(axisStart,p), axisDir) ) ); }
